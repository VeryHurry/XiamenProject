...
+ (NSData *)handshakeMessagePacketData {
    ...
    // rsa加密
    NSData *enData = [RSAM encryptData:writerPacket.data publicKey:pubkey];
 ...
 *  握手成功响应的bodyData
 *
 *  @param body_data 读到的握手ok的bodyData
 *
...
//    RFIWriter *writerPacket = [RFIWriter writerWithData:bodyData];
//
//    NSDictionary *bodyDic = @{@"pageNum":@"101",@"imei":@"123456"};
//    NSData *dicData = [NSJSONSerialization dataWithJSONObject:bodyDic options:NSJSONWritingPrettyPrinted error:nil];
//    NSString *bodyStr = [[NSString alloc]initWithData:dicData encoding:NSUTF8StringEncoding];
//    [writerPacket writeString:bodyStr];
//
//    //    拼接packet
//    NSMutableData *ipHeaderData = [MessageDataPacketTool ipHeaderWithLength:(uint32_t)writerPacket.data.length cmd:MpushMessageBodyCMDHeartBeat cc:0 flags:0 sessionId:1 lrc:0];
//    [ipHeaderData appendData:writerPacket.data];
//
//
//    return ipHeaderData;
}
...
//    NSMutableDictionary *contentDict = [NSMutableDictionary dictionary];
    ...
    //headers
    NSString *headersStr =[NSString stringWithFormat:@"Content-Type:application/json\ncharset:UTF-8\ndeviceTypeId:1\nreadTimeout:10000"];
 ...
 */
+ (NSDictionary *)chatDataSuccessWithData:(NSData *)bodyData{
...
//    NSLog(@"str--%@",strsss);
    ...
    if (strsss) {
        NSData *jsonData = [strsss dataUsingEncoding:NSUTF8StringEncoding];
...
//    NSString *deviceId = @"123456";
    ...
    // rsa加密
    NSData *enData = [RSAM encryptData:writer.data publicKey:pubkey];
