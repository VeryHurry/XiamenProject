...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat yAxisOffset =  10.0f;
    CGPoint point;
    CGFloat yStepHeight = rect.size.height / self.lineChartYLabelArray.count;

    CGContextSetStrokeColorWithColor(ctx, [UIColor lightGrayColor].CGColor);
    CGContextSetFillColorWithColor(ctx, [UIColor redColor].CGColor);
    
    for (NSUInteger i = 0; i < _lineChartYLabelArray.count; i++) {
        point = CGPointMake(10 + yAxisOffset, (rect.size.height - i * yStepHeight -5));
...
- (void)setLineChartXLabelArray:(NSArray *)lineChartXLabelArray{
   ...
   // CGFloat labelWidthScale = (self.frame.size.width-leftXMarginScale-rightXMarginScale)/_lineChartXLabelArray.count;
    self.totalWidth = kScreen_W-20-30*Scale;
    NSInteger aryCount = _lineChartXLabelArray.count;
    CGFloat labelWidthScale = self.totalWidth/aryCount;
    
    for (int idx = 0; idx < aryCount; idx ++) {
        
        CGFloat x = 20*Scale + idx*labelWidthScale;
...
- (void)setLineChartDataArray:(NSArray *)LineChartDataArray{
   ...
   // [self drawGragient];
    ...
    layers.strokeColor = [UIColor colorWithHexStr:@"#FFDA45"].CGColor;
...
- (UIBezierPath *)getPath{
    ...
    for (int idx =0; idx<_LineChartDataArray.count; idx++) {
        if (idx == 0) {
            ...
            CGPoint startPoint = CGPointMake([_pointXArray[0] floatValue], self.frame.size.height- [_LineChartDataArray[0] floatValue]*2*Scale -bottomMarginScale*Scale);
            [bezierPath moveToPoint:startPoint];
            [self.topPointArray addObject:[NSValue valueWithCGPoint:startPoint]];
            
        }else{
            
            CGPoint point = CGPointMake([_pointXArray[idx] floatValue], self.frame.size.height- [_LineChartDataArray[idx] floatValue]*2*Scale - bottomMarginScale*Scale);
...
- (void)drawGredientLayer{
    ...
    gradientLayer.frame = CGRectMake(0, 0, self.totalWidth+30*Scale, self.frame.size.height-bottomMarginScale*Scale);
    gradientLayer.colors = @[(__bridge id)[UIColor colorWithRed:255/255.0 green:244/255.0 blue:212/255.0 alpha:0.8].CGColor,(__bridge id)[UIColor colorWithRed:255/255.0 green:244/255.0 blue:212/255.0 alpha:0.8].CGColor];
    ...
    for (int i=0; i<_LineChartDataArray.count; i++) {
        [gradientPath addLineToPoint:CGPointMake([_pointXArray[i] floatValue], self.frame.size.height- [_LineChartDataArray[i] floatValue]*2*Scale -bottomMarginScale*Scale)];
    }
    [gradientPath addLineToPoint:CGPointMake([_pointXArray[_pointXArray.count-1] floatValue], self.frame.size.height-bottomMarginScale*Scale)];
