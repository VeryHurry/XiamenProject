...
//            取数组中最大的值
            ...
            self.leftSpace = 30*Scale;
            self.topSpace = 285*Scale - max_number*2*Scale;
            self.bottomSpace = 15*Scale;
            self.itemW = 20*Scale;
...
//    底部线
    ...
    {
        UILabel *label = [self createLabelWithFrame:CGRectMake(0, 0, 40*Scale, self.bottomSpace) text:self.nameArray[i]];
    ...
    {
        UILabel *label = [self createLabelWithFrame:CGRectMake(0, 0, self.leftSpace, 10*Scale) text:@""];
...
{   
    for (int i = 0; i < self.countArray.count; i++)
    {
        ...
        layer.strokeColor = [UIColor colorWithRed:255.0/255.0f green:220.0/255.0f blue:90.0/255.0f alpha:1.00f].CGColor;
...
//        贝塞尔二次曲线
        ...
        UICountingLabel *label = [[UICountingLabel alloc] initWithFrame:CGRectMake(0, 0, 30*Scale, 16*Scale)];
        label.font = [UIFont systemFontOfSize:10*Scale];
        label.textColor = [UIColor colorWithRed:0.35f green:0.35f blue:0.37f alpha:1.00f];
        label.textAlignment = NSTextAlignmentCenter;
