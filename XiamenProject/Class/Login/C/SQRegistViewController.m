...
@interface SQRegistViewController ()
...
@implementation SQRegistViewController
...
- (IBAction)getSMS:(id)sender
{
    ...
    [self openCountdown];
    NSDictionary *parameters = @{@"phone":_telText.text};
    
    if ([Base_AFN_Manager isNetworking]) {
        
        [Base_AFN_Manager postUrl:IP_SPLICE(IP_GETSMS) parameters:parameters success:^(id success) {
...
- (IBAction)next:(id)sender
{
    ...
    SQRegistPSWViewController *vc = [[UIStoryboard storyboardWithName:@"Login" bundle:nil] instantiateViewControllerWithIdentifier:@"setPSW_sb"];
    vc.codeStr = _code;
