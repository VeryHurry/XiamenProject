...
#define kNomalColor  [UIColor whiteColor]
...
- (instancetype)init{
    if (self = [super init]) {
        self.frame = CGRectMake(0, 15, [UIScreen mainScreen].bounds.size.width, 38);
        [self initData];
    }
    return self;
}
...
- (void)initData
{
    ...
    _index = 0;
    CGFloat leftSpacing = ([UIScreen mainScreen].bounds.size.width - 24-width*num-spacing*(num-1))/2.0;
    for (int i = 0; i < num; i ++) {
        CGRect frame = CGRectMake(leftSpacing + i*(width + spacing), 0, width, 38);
