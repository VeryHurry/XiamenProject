...
@interface SQScanViewController ()
{
    NSTimer *_timer;
    int num;
    BOOL upOrDown;
    ...
    if ([metadataObjects count] > 0) {
        ...
        if ([metadataObject isKindOfClass:[AVMetadataMachineReadableCodeObject class]]) {
            ...
            if (stringValue != nil) {
                ...
                NSInteger type = [[NSUserDefaults standardUserDefaults] integerForKey:@"type"];
                
                if (type == 1) {
                    SQScanResultViewController *vc = [SQScanResultViewController new];
                    vc.qrCode = stringValue;
                    [self.navigationController pushViewController:vc animated:YES];
                }
                else
                {
                    SQVehicleMonitoringViewController *vc = [SQVehicleMonitoringViewController new];
                    vc.qrCode = stringValue;
                    [self.navigationController pushViewController:vc animated:YES];
